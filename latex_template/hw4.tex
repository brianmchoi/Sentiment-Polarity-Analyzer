...
\newcommand{\dueDate}{Wednesday, October 14, 2020 11:59 PM}
...
{\Large \question Logistic Regression and Stochastic Gradient Descent}

\begin{parts}
        \part[1] Which of the following are true about logistic regression?
    ...
    \part[1] The negative log-likelihood $J(\thetav)$ for binary logistic regression can be expressed as 
...
\begin{parts}
    \part[1] Calculate $J(\thetav$), $\frac{1}{N}$ times the negative log-likelihood over the given data after iteration $n$ (Note here we are using natural log, i.e., the base is $e$). 
    ...
    \part[2] Calculate the gradients $\frac{\partial J(\thetav)}{\partial \thetav_j}$ with respect to $\theta_{j}$, for all $j \in \{1, 2, 3, 4, 5\}$
    ...
    \part[1] Update the parameters following the parameter update step $\thetav_j \leftarrow \thetav_j - \alpha \frac{\partial J(\thetav)}{\partial \thetav_j}$ and give the final (numerical) value of the vector $\theta$. use learning rate $\alpha = 1$.
    ...
    \part[2] The following table shows the sparse feature representation for the given data
    \begin{table}[h]
    \centering
    \begin{tabular}{cll}
    \toprule
...
{\Large \question Multinomial Logistic Regression}
...
\begin{parts}
    \part[1] To motivate multinomial logistic regression, we will first look at a general way to extend a binary classifier to a multiclass classifier and apply it to logistic regression. Which of the following are the possible ways to determine the class for each unlabelled data point $\xv^{(i)}$? No need to worry about tie breaking.
    ...
    \begin{list}{}
        ...
        \item 
            \emptysquare
            % \filledsquare
            Train $K$ classifiers. Each trained classifier $h_k(\xv^{(i)})$  will determine if a point $\xv^{(i)}$ is in class $k$ or not for $k = 1, \ldots, K$. The class that has the highest probability from the $K$ classifiers will be the predicted label of point $\xv^{(i)}$
        \item 
            \emptysquare
            % \filledsquare
            Train $K$ classifiers. Each trained classifier $h_k(\xv^{(i)})$  will determine if a point $\xv^{(i)}$ is in class $k$ or not for $k = 1, \ldots, K$. The class that has the highest 'confidence score' from the K classifiers will be the predicted label of point $\xv^{(i)}$, where a 'confidence score' of classifier $k$ is defined as $\wv^T_k \xv^{(i)} + b_k$
        \item 
            \emptysquare
            % \filledsquare
            Train $\frac{K(K-1)}{2}$ classifiers. Each trained classifier $h_{k,j}(\xv^{(i)})$  will determine if a point $\xv^{(i)}$ is in class $k$ or class $j$ for all unique combinations of $k$ and $j$, where $k \neq j$, i.e. for $k \in \{1, \ldots, K\}$ and  $j \in \{k+1, \ldots, K\}$. The class label that is predicted the most number of times by all classifiers is assigned to point $\xv^{(i)}$
        \item 
            \emptysquare
            % \filledsquare
            None of the above
    ...
    \part[2]
    \label{sec:multinomial-likelihood}
    Now we would like a method to do multiclass classification by training a single classifier that considers all classes simultaneously. Multinomial logistic regression is such a method. Remember that in multinomial logistic regression, the probability of random variable $Y$ taking on class value $k$ given the input data point $\xv$ is:
    \begin{equation} \label{eq:1}
        P\left(Y = k \mid \xv, \Thetav\right) = \frac{\exp\left(\Thetav_k \xv \right)}{\sum_{q = 1}^K \exp\left(\Thetav_{q} \xv \right)} = \text{softmax}(\Thetav \xv)_k
    \end{equation}
    where $\Thetav$ is the parameter matrix of size $K \times (M + 1)$, and $\Thetav_k$ denotes the $k$-th \textbf{row} of $\Thetav$, which is the parameter vector for the $k$-th class. Assume that we have folded the bias term into $\Thetav$, so we have $\xv^{(i)} \in \Rb^{M + 1}$. 
    
    The likelihood that the $i$-th training label takes on the observed label $y^{(i)}$ given the $i$-th input data point is:
    \begin{equation} \label{eq:2}
        P\left(Y^{(i)} = y^{(i)} \mid \xv^{(i)}, \Thetav\right) = \frac{\exp\left(\Thetav_{y^{(i)}} \xv^{(i)} \right)}{\sum_{j = 1}^K \exp\left(\Thetav_{j} \xv^{(i)} \right)} = \text{softmax}(\Thetav \xv^{(i)})_{y^{(i)}}
    \end{equation}
    ...
    \begin{equation} \label{eq:3}
        \mathcal{L}(\Thetav; \yv, \Xv) = p(y^{(1)}, \cdots, y^{(N)} \mid \xv^{(1)}, \cdots, \xv^{(N)} ,\Thetav) = \prod_{i=1}^N P\left(Y^{(i)}=y^{(i)} \mid \xv^{(i)}, \Thetav \right)
    \end{equation}

    Recall from binary logistic regression that we considered both $P(Y=1 \mid \xv)$ and $P(Y=0 \mid \xv)$ cases in the conditional likelihood formulation by raising each factor to the appropriate power of $y^{(i)}$:
    \begin{equation} \label{eq:binary_likelihood}
        \mathcal{L}_{\text{binary}}(\thetav; \yv, \Xv) = \prod_{i=1}^N p\left(1 \mid \xv^{(i)}, \thetav \right)^{y^{(i)}}P\left(0 \mid \xv^{(i)}, \thetav \right)^{\left(1-y^{(i)}\right)}
    \end{equation}

    This won't quite work with multinomial logistic regression because $y^{(i)} \in \{1 \cdots K\}$ rather than $\{0,1\}$. So, we will make use of the indicator function $\Ib(y^{(i)}=k)$ to generate a 1 when the label of the $i$-th point is class $k$ and a 0 otherwise. For convenience (and to avoid indicator function notation everywhere), we can convert all of the label data $y^{(i)}$ into a binary matrix $\Tv$ of size $N \times K$, where the $i,j$ entry of $\Tv$ is $T_{ij}=\Ib(y^{(i)}=j)$. Note that each row of $\Tv$ will have exactly one 1.
    
    % Let $\Xv$ be the $N \times (M+1)$ matrix where the $i$-th row is $x^{(i)}$.
    
    Now, write the conditional likelihood $ \mathcal{L}(\Thetav; \Tv, \Xv)$ from Equation \ref{eq:3} in terms of $N$, $K$, $T_{ij}$ and $p\left(j \mid \xv^{(i)}, \Thetav \right)$.
    ...
    \part[2] Write the objective function, $J(\Thetav)$, as $\frac{1}{N}$ times the \emph{negative} conditional log-likelihood in terms of $N$, $K$, $T_{ij}$ and $p\left(j \mid x^{(i)}, \Thetav \right)$. This objective function is also known as cross-entropy loss. To help you with the next part, write down the objective function after replacing $p\left(j \mid x^{(i)}, \Thetav \right)$ using equation \ref{eq:2} given in the previous part. Do not include the literal term "softmax" in your answer.
    \label{sec:multinomial-loglikelihood}
    
    \begin{your_solution}[height=14cm]
    % YOUR ANSWER 
    \end{your_solution}

    \clearpage
    
    \part[5] Now let's derive the partial derivative of the objective function with respect to the $k$th parameter vector $\Thetav_k$. That is, derive $\frac{ \partial J(\Thetav) }{ \partial \Thetav_k}$, where $J(\Thetav)$ is the objective function that you provided above. Show that the partial derivative  is as follows:

    $$
    \frac{\partial J(\Thetav)}{\partial \Thetav_k} = - \frac{1}{N} \sum_{i = 1}^N \left(T_{ik} - p\left(k \mid \xv^{(i)}, \Thetav\right)\right)  \xv^{(i)}
    $$
    
    Show all steps of the derivation. (\textbf{Hint:} A good first step would be to simplify your answer from part \ref{sec:multinomial-loglikelihood} as much as you can, if you haven't already done so in the previous part)
    ...
    \part[2] Write pseudocode corresponding to the stochastic gradient descent \textbf{update steps} for an arbitrary $\Thetav_k$ for $k = 1, \ldots, K$, using the $i$-th training example in terms of $\xv^{(i)}$, $T_{ik}$, $p\left(k \mid \xv^{(i)}, \Thetav\right)$, and learning rate $\alpha$. 
    ...
    \part[1] How does regularization such as $\ell_1$ and $\ell_2$ help correct the problem?
...
% YOUR ANSWER 
% \begin{center}
% \includegraphics[width = 0.8\textwidth]{plot_model1.png}
% \end{center}
\end{your_solution}
...
% YOUR ANSWER 
% \begin{center}
% \includegraphics[width = 0.8\textwidth]{plot_model2.png}
% \end{center}
\end{your_solution}
